...
public class NPCManager : MonoBehaviour
{
    ...
    void RandomSpawnCAR ()
    {
        ...
        Transform CARTransform = Instantiate(
            ...
            DetermineSpawnLocation(CARspawnPoints[0].position + new Vector3(-20f, -0.5f + UnityEngine.Random.Range(0.1f, 0.4f), -20f)),
            Quaternion.identity
                );
            ...
            if (NPCFlip[i])
            {
                if (NPCList[i].pos.gameObject.tag == "CAR")
                {
                    ...
                    if (NPCList[i].speed < 0)
                        NPCList[i].pos.position = DetermineSpawnLocation(new Vector3(NPCList[i].pos.position.x, NPCList[i].pos.position.y - 3f, NPCList[i].pos.position.z));
                    else
                        NPCList[i].pos.position = DetermineSpawnLocation(new Vector3(NPCList[i].pos.position.x, NPCList[i].pos.position.y + 3f, NPCList[i].pos.position.z));
    ...
    Vector3 DetermineSpawnLocation (Vector3 point)
    {
        if (CheckIfSpawnPointEmpty(point))
            return point;
        else
            if (point.x >= 30)
                return point + new Vector3(5, 0, -20);
            else
                return point - new Vector3(5, 0, -20);
    ...
    bool CheckIfSpawnPointEmpty (Vector3 point)
    {
        Collider2D[] obstaclesFound = Physics2D.OverlapBoxAll(new Vector2(point.x, point.y), new Vector2(5, 1), 0f);
        return obstaclesFound.Length == 0;
    }
}
